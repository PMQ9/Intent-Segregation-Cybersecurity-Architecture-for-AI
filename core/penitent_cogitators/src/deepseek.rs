...
Never include any other text."#
    ...
    fn parse_response(&self, content: &str) -> Result<CorruptionAnalysis, CogitatorError> {
        serde_json::from_str::<CorruptionAnalysis>(content)
            .map_err(|e| CogitatorError::DetectionError(format!("Failed to parse DeepSeek JSON: {}", e)))
    }
}
...
impl SacrificialCogitator for DeepSeekCogitator {
    async fn test_for_corruption(
        ...
        let content = deepseek_response
            .choices
            .first()
            .ok_or_else(|| CogitatorError::DetectionError("No choices in DeepSeek response".to_string()))?
            .message
            .content
            .clone();
